...
wordstring = PlaintextWriter.write(doc).read()

# Removes punctuation
replace_punctuation = string.maketrans(string.punctuation,
                                       ' '*len(string.punctuation))
wordstring = wordstring.translate(replace_punctuation)
...
replace_esc = string.maketrans(escchars,
                               ' '*len(escchars))
wordstring = wordstring.translate(replace_esc)


stringlist = wordstring.decode('utf-8').split()
stringlist = wordstring.split()
wordfreq = []
wordlist = []
worddict = {}
for w in stringlist:
    if len(w) < 50 and w not in stopwords:
        wordlist.append(w)
        wordfreq.append(wordlist.count(w))
        worddict[w] = wordlist.count(w)

# print("Pairs\n" + str(zip(wordlist, wordfreq)))
# print sorted(worddict, key=worddict.__getitem__)
print worddict
