...
def xn4parse(xmlfilename):
	...
	#		print "Found a grandchild"
			...
			DerivXn[10] = textattribute(child2.find('conversionOrExercisePrice/value'))
			DerivXn[11] = textattribute(child2.find('transactionDate/value'))
			DerivXn[12] = textattribute(child2.find('transactionCoding/transactionCode'))
			DerivXn[13] = floattextattribute(child2.find('transactionAmounts/transactionShares/value'))
			DerivXn[14] = floattextattribute(child2.find('transactionAmounts/transactionPricePerShare/value'))
			DerivXn[15] = textattribute(child2.find('transactionAmounts/transactionAcquiredDisposedCode/value'))
			DerivXn[16] = textattribute(child2.find('expirationDate/value'))
			DerivXn[17] = textattribute(child2.find('underlyingSecurity/underlyingSecurityTitle/value'))
			DerivXn[18] = floattextattribute(child2.find('underlyingSecurity/underlyingSecurityShares/value'))
			DerivXn[19] = floattextattribute(child2.find('postTransactionAmounts/sharesOwnedFollowingTransaction/value'))
			DerivXn[20] = textattribute(child2.find('ownershipNature/directOrIndirectOwnership/value'))
...
def xn3parse(xmlfilename):
	...
	root = tree.getroot()
	#The root object has child nodes 
	#below nested loops iterate to find the non-derivative and then derivative transactions ("NonDerivHs")
	#First it defines the list then it constructs each transaction in order then it adds them to the list
	...
	#This finds each transaction, making a list of its attributes
		for  child2 in child.findall('nonDerivativeHolding'):
			...
			NonDerivH[3] = textattribute(root.find('reportingOwner/reportingOwnerId/rptOwnerName'))
			NonDerivH[4] = textattribute(root.find('reportingOwner/reportingOwnerRelationship/isDirector'))
			NonDerivH[5] = textattribute(root.find('reportingOwner/reportingOwnerRelationship/isOfficer'))
			NonDerivH[6] = textattribute(root.find('reportingOwner/reportingOwnerRelationship/isTenPercentOwner'))
	...
	#		print "Found a grandchild"
			...
			DerivH[3] = textattribute(root.find('reportingOwner/reportingOwnerId/rptOwnerName'))
			DerivH[4] = textattribute(root.find('reportingOwner/reportingOwnerRelationship/isDirector'))
			DerivH[5] = textattribute(root.find('reportingOwner/reportingOwnerRelationship/isOfficer'))
			DerivH[6] = textattribute(root.find('reportingOwner/reportingOwnerRelationship/isTenPercentOwner'))
			DerivH[7] = textattribute(root.find('reportingOwner/reportingOwnerRelationship/isOther'))
